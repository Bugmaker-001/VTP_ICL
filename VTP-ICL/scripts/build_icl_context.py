...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate class descriptions from visualized samples and build ICL context."
    )
    ...
    messages = [
        {
            ...
            "content": [
                {
                    "text": (
                        "Given two visualized fabric-defect images from the same class, "
                        "write one short discriminative visual description for this class. "
                        "Output one English paragraph only."
                    )
                },
                {"text": f"Class: {label}"},
...
def main() -> None:
    ...
    print(f"ICL context built for {len(contexts)} classes")
    print(f"Output file: {Path(args.output_json).resolve()}")
